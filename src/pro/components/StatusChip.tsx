export function StatusChip({ status, size = 'sm', label: labelOverride }: { status: string; size?: 'sm' | 'md'; label?: string }) {
  const styles: Record<string, string> = {
    ...
    ready_for_pickup: 'bg-green-50 text-green-700 border-green-100',
    ...
    delivering: 'bg-violet-50 text-violet-700 border-violet-100',
    picked_up: 'bg-violet-50 text-violet-700 border-violet-100',
    delivered: 'bg-ink/5 text-ink/60 border-line',
    collected: 'bg-ink/5 text-ink/60 border-line',
