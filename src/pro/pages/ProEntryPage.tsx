...
export default function ProEntryPage() {
  const { role, login, verifySellerLogin } = usePro();
  ...
  if (selectedRole) {
    return (
      <div className="min-h-screen bg-cream flex items-center justify-center p-4">
        <LoginForm role={selectedRole} onBack={() => setSelectedRole(null)} onLogin={(id, name, shopInfo) => login(selectedRole, id, name, shopInfo)} verifySeller={verifySellerLogin} />
      </div>
    );
  }
