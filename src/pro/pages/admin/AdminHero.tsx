...
const emptyForm: HeroSlideInput = { imageUrl: '', title: '', subtitle: '', ctaLabel: '', ctaLink: '', sortOrder: 0, isActive: true };
...
export default function AdminHero() {
  ...
  const move = async (row: HeroSlideRow, direction: -1 | 1) => {
    const sorted = [...rows].sort((a, b) => a.sort_order - b.sort_order);
    const idx = sorted.findIndex((r) => r.id === row.id);
    const swapWith = sorted[idx + direction];
    if (!swapWith) return;
    await Promise.all([
      updateHeroSlide(row.id, { ...rowToInput(row), sortOrder: swapWith.sort_order }),
      updateHeroSlide(swapWith.id, { ...rowToInput(swapWith), sortOrder: row.sort_order }),
    ]);
    await load();
  };

  const toggleActive = async (row: HeroSlideRow) => {
    await updateHeroSlide(row.id, { ...rowToInput(row), isActive: !row.is_active });
    await load();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-2xl font-semibold text-ink">Hero de la Home</h1>
          <p className="mt-1 text-sm text-ink/55">Gérez les slides affichés en haut de la page d'accueil.</p>
        </div>
        <button onClick={openNew} className="btn-primary flex items-center gap-1.5"><Plus size={16} /> Ajouter un slide</button>
      </div>
      ...
      {loading ? (
        ...
        <div className="card p-8 text-center text-sm text-ink/50">Aucun slide pour l'instant. La Home affiche son contenu par défaut.</div>
      ) : (
        <div className="space-y-3">
          {sortedRows.map((row, i) => (
            <div key={row.id} className="card flex items-center gap-4 p-4">
              <div className="h-16 w-28 flex-shrink-0 overflow-hidden rounded-lg bg-cream">
                {row.image_url ? <SmartImage src={row.image_url} alt="" className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center text-ink/25"><ImageIcon size={20} /></div>}
              </div>
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-semibold text-ink">{row.title}</p>
                {row.subtitle && <p className="truncate text-xs text-ink/50">{row.subtitle}</p>}
                <p className="mt-0.5 text-xs text-ink/40">{row.cta_label} → {row.cta_link}</p>
              </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/30 p-4" onClick={() => setShowForm(false)}>
          <div className="card max-h-[90vh] w-full max-w-lg overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
            <h2 className="font-display text-lg font-semibold text-ink">{editingId ? 'Modifier le slide' : 'Nouveau slide'}</h2>
...
function rowToInput(row: HeroSlideRow): HeroSlideInput {
  return {
    imageUrl: row.image_url,
    title: row.title,
    subtitle: row.subtitle ?? '',
